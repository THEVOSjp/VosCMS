...
<div class="group bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 overflow-hidden hover:shadow-md hover:border-indigo-300 dark:hover:border-indigo-600 transition-all">
    ...
    <a href="<?= $_itemUrl ?>" class="block aspect-[16/9] bg-gradient-to-br from-<?= $_color ?>-50 to-<?= $_color ?>-100 dark:from-<?= $_color ?>-900/20 dark:to-<?= $_color ?>-800/20 flex items-center justify-center relative overflow-hidden">
        ...
        <div class="absolute top-2 left-2 flex gap-1.5">
            ...
            <span class="px-2 py-0.5 text-xs font-medium rounded-full bg-yellow-100 text-yellow-700 dark:bg-yellow-900/40 dark:text-yellow-400">
                <?= __mp('featured') ?>
            </span>
            <?php endif; ?>
        </div>
    ...
    <div class="p-4">
        ...
        <div class="flex items-center justify-between mt-3">
            ...
            <div>
                ...
                <?php elseif ($_onSale): ?>
                <span class="text-xs text-zinc-400 line-through mr-1"><?= number_format($_price, $_currency === 'KRW' || $_currency === 'JPY' ? 0 : 2) ?></span>
                <span class="text-sm font-bold text-red-600 dark:text-red-400"><?= number_format($_effectivePrice, $_currency === 'KRW' || $_currency === 'JPY' ? 0 : 2) ?> <?= $_currency ?></span>
                <?php else: ?>
                <span class="text-sm font-bold text-zinc-800 dark:text-zinc-200"><?= number_format($_price, $_currency === 'KRW' || $_currency === 'JPY' ? 0 : 2) ?> <?= $_currency ?></span>
                <?php endif; ?>
            </div>
