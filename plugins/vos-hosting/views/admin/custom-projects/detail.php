...
<div class="px-4 sm:px-6 lg:px-8 py-6 max-w-6xl mx-auto">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-4">
                ...
                <div class="p-5">
                    ...
                    <?php else: foreach ($quotes as $q):
                        $qBg = match ($q['status']) {
                            ...
                            'sent' => 'bg-amber-50 dark:bg-amber-900/10 border-amber-300 dark:border-amber-700',
        ...
        <div class="space-y-4">
            ...
            <!-- 내부 메모 -->
            <div class="bg-yellow-50 dark:bg-yellow-900/10 rounded-xl border border-yellow-200 dark:border-yellow-800 p-4">
                <p class="text-xs font-bold text-yellow-700 dark:text-yellow-400 mb-2">📝 <?= htmlspecialchars(__('services.admin_custom.section_admin_note')) ?></p>
                <textarea id="adminNote" rows="6" class="w-full px-2 py-1.5 text-xs border border-yellow-300 dark:border-yellow-700 dark:bg-zinc-700 dark:text-white rounded" placeholder="<?= htmlspecialchars(__('services.admin_custom.admin_note_ph')) ?>"><?= htmlspecialchars($project['admin_note'] ?? '') ?></textarea>
                <button onclick="saveAdminNote()" class="mt-2 w-full px-3 py-1.5 text-xs font-medium text-white bg-yellow-600 hover:bg-yellow-700 rounded">
