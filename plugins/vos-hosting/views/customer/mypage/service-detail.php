...
// 1:1 상담 탭 — 기술 지원 active 부가서비스 보유 시 노출
$_techSupportSub = null;
foreach ($servicesByType['addon'] ?? [] as $_aSub) {
    if (($_aSub['status'] ?? '') !== 'active') continue;
    $_lbl = (string)($_aSub['label'] ?? '');
    if (stripos($_lbl, '기술 지원') !== false
        || stripos($_lbl, 'tech support') !== false
        || stripos($_lbl, '技術サポート') !== false) {
        $_techSupportSub = $_aSub;
        break;
    }
}
$_supportUnreadCount = 0;
if ($_techSupportSub && $_hasHosting) {
    $servicesByType['support'] = [$_techSupportSub];
    try {
        $_uSt = $pdo->prepare("SELECT COUNT(*) FROM {$prefix}support_tickets WHERE user_id = ? AND unread_by_user = 1 AND status != 'closed'");
        $_uSt->execute([$user['id']]);
        $_supportUnreadCount = (int)$_uSt->fetchColumn();
    } catch (\Throwable $e) {}
}
    ...
    <div class="border-b border-zinc-200 dark:border-zinc-700 mb-6">
        <nav class="flex gap-1 -mb-px overflow-x-auto">
            ...
            <button onclick="showServiceTab('<?= $type ?>')" id="tab_<?= $type ?>"
                ...
                <?php if ($type === 'support'): ?>
                    <?php if ($_supportUnreadCount > 0): ?>
                    <span class="text-[10px] px-1.5 py-0.5 bg-red-500 text-white rounded-full font-bold"><?= $_supportUnreadCount ?></span>
                    <?php endif; ?>
                <?php else: ?>
