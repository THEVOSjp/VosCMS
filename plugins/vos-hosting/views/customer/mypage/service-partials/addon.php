...
<?php
// 기술 지원 sub 활성 여부 — 1:1 상담 섹션 노출 조건
$_supportActive = false;
$_supportSubId = null;
foreach ($subs as $_s) {
    if (($_s['status'] ?? '') === 'active' && stripos((string)$_s['label'], '기술 지원') !== false) {
        $_supportActive = true;
        $_supportSubId = (int)$_s['id'];
        break;
    }
    // 영문/일본어 라벨 매칭도
    if (($_s['status'] ?? '') === 'active' && (stripos((string)$_s['label'], 'tech support') !== false || stripos((string)$_s['label'], '技術サポート') !== false)) {
        $_supportActive = true;
        $_supportSubId = (int)$_s['id'];
        break;
    }
}
if ($_supportActive && $_hostSub):
?>
<div class="bg-white dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700 overflow-hidden mt-3" id="supportTicketsSection">
...
<div id="supportThreadModal" class="hidden fixed inset-0 z-[100] items-center justify-center p-4">
    ...
    <div class="relative bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div id="supportThreadMessages" class="p-6 space-y-3 max-h-[60vh] overflow-y-auto"></div>
...
<script>
(function(){
    var spHostSubId = <?= (int)$_hostSub['id'] ?>;
    var spAddonSubId = <?= (int)($_supportSubId ?? 0) ?>;
    var spPage = 1;
    ...
    window.supportSubmitNew = function() {
        ...
        btn.disabled = true;
        serviceAction('support_create_ticket', {
        ...
        serviceAction('support_get_ticket', { ticket_id: ticketId })
            .then(function(d) {
                ...
                (d.messages || []).forEach(function(m) {
                    ...
                    var sender = isAdmin ? '<?= htmlspecialchars(__('services.detail.support_admin')) ?>' : (m.sender_name || m.sender_email || 'user');
    ...
    window.supportCloseThreadModal = function() {
        ...
        m.classList.add('hidden'); m.classList.remove('flex');
        document.body.style.overflow = '';
        loadList(spPage); // 새로고침 (unread 갱신)
    };
    window.supportSubmitReply = function() {
        ...
        btn.disabled = true;
        serviceAction('support_post_message', { ticket_id: spCurrentTicket.id, body: body })
            .then(function(d) {
                btn.disabled = false;
                if (d.success) {
