...
<div class="space-y-4">
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700 overflow-hidden">
        ...
        <div class="p-5">
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm">
                ...
                <div>
                    <p class="text-[10px] text-zinc-400 uppercase tracking-wider mb-0.5"><?= htmlspecialchars(__('services.detail.f_monthly_fee')) ?></p>
                    <p class="font-medium text-zinc-700 dark:text-zinc-300"><?= (int)$sub['unit_price'] > 0 ? $fmtPrice($sub['unit_price'], $sub['currency']) . __('services.order.hosting.price_per_month') : __('services.order.summary.free') ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-zinc-400 uppercase tracking-wider mb-0.5"><?= htmlspecialchars(__('services.detail.f_next_inspection')) ?></p>
                    <p class="font-medium text-zinc-700 dark:text-zinc-300"><?= htmlspecialchars($meta['next_check'] ?? '-') ?></p>
                </div>
                <div>
                    <p class="text-[10px] text-zinc-400 uppercase tracking-wider mb-0.5"><?= htmlspecialchars(__('services.detail.f_last_inspection')) ?></p>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl border border-gray-200 dark:border-zinc-700 p-5">
        <p class="text-[10px] font-bold text-zinc-500 dark:text-zinc-400 uppercase tracking-wider mb-3"><?= htmlspecialchars(__('services.detail.work_history')) ?></p>
