...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.domain.section_free') ?>
        </h3>
    </div>
    <div class="px-6 py-4">
        <p class="text-xs text-zinc-400 mb-3"><?= __('services.admin.domain.free_hint') ?></p>
        ...
        <div class="flex items-center gap-2 mt-3">
            <input type="text" id="newFreeDomain" placeholder="<?= htmlspecialchars(__('services.admin.domain.placeholder_free'), ENT_QUOTES) ?>" class="<?= $_inp ?> text-xs max-w-[180px] font-mono" onkeydown="if(event.key==='Enter'){event.preventDefault();addFreeDomain();}">
            <button type="button" onclick="addFreeDomain()" class="text-xs text-green-600 dark:text-green-400 hover:underline"><?= __('services.admin.domain.add_button') ?></button>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.domain.section_blocked') ?>
        </h3>
    </div>
    <div class="px-6 py-4">
        <p class="text-xs text-zinc-400 mb-3"><?= __('services.admin.domain.blocked_hint') ?></p>
        ...
        <div class="flex items-center gap-2 mt-3">
            <input type="text" id="newBlockedSub" placeholder="<?= htmlspecialchars(__('services.admin.domain.placeholder_blocked'), ENT_QUOTES) ?>" class="<?= $_inp ?> text-xs max-w-[150px] font-mono" onkeydown="if(event.key==='Enter'){event.preventDefault();addBlockedSub();}">
            <button type="button" onclick="addBlockedSub()" class="text-xs text-red-600 dark:text-red-400 hover:underline"><?= __('services.admin.domain.add_button') ?></button>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.domain.section_search_tlds') ?>
        </h3>
    </div>
    <div class="px-6 py-4">
        ...
        <div class="flex items-center gap-2 mt-3">
            <input type="text" id="newSearchTLD" placeholder="<?= htmlspecialchars(__('services.admin.domain.placeholder_search_tld'), ENT_QUOTES) ?>" class="<?= $_inp ?> text-xs max-w-[120px] font-mono" onkeydown="if(event.key==='Enter'){event.preventDefault();addSearchTLD();}">
            <button type="button" onclick="addSearchTLD()" class="text-xs text-cyan-600 dark:text-cyan-400 hover:underline"><?= __('services.admin.domain.add_button') ?></button>
        </div>
        <p class="text-xs text-zinc-400 mt-2"><?= __('services.admin.domain.search_hint') ?></p>
    </div>
</div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700 flex items-center justify-between">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.domain.section_pricing') ?>
        </h3>
        <div class="flex items-center gap-3">
            <span class="text-[10px] text-zinc-400"><?= __('services.admin.domain.exchange_rate_label') ?>: 1 USD = <span id="domainExRate"><?= number_format($_dispRate, 2) ?></span> <?= $_dispCur ?></span>
            <button type="button" onclick="fetchNamesiloPrices()" id="btnFetchNS" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-orange-600 dark:text-orange-400 bg-orange-50 dark:bg-orange-900/30 hover:bg-orange-100 dark:hover:bg-orange-900/50 rounded-lg transition">
                ...
                <?= __('services.admin.domain.btn_namesilo') ?>
            </button>
            <button type="button" onclick="fetchXdomainPrices()" id="btnFetchDomainPrices" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-purple-600 dark:text-purple-400 bg-purple-50 dark:bg-purple-900/30 hover:bg-purple-100 dark:hover:bg-purple-900/50 rounded-lg transition">
                ...
                <?= __('services.admin.domain.btn_xdomain') ?>
            </button>
            <button type="button" onclick="importAllXdomainTLDs()" id="btnImportAllTLD" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-900/30 hover:bg-indigo-100 dark:hover:bg-indigo-900/50 rounded-lg transition">
                ...
                <?= __('services.admin.domain.btn_import_all') ?>
            </button>
        </div>
    </div>
    <div class="p-6 space-y-4">
            ...
            <table class="w-full text-sm" id="tblDomains">
                <thead>
                    <tr class="text-[11px] text-zinc-400 dark:text-zinc-500 border-b border-zinc-200 dark:border-zinc-600">
                        <th class="pb-2 px-1 w-8" rowspan="2"><input type="checkbox" checked onchange="document.querySelectorAll('.dom-active').forEach(c=>c.checked=this.checked)" class="rounded text-blue-600 cursor-pointer" title="<?= htmlspecialchars(__('services.admin.domain.col_select_all_title'), ENT_QUOTES) ?>"></th>
                        ...
                        <th class="pb-2 px-1 text-center w-24"><?= __('services.admin.domain.col_member_price') ?> (<?= $_dispCur ?>)</th>
                        <th class="pb-2 px-1 text-center w-24"><?= __('services.admin.domain.col_normal_price') ?> (<?= $_dispCur ?>)</th>
                        <th class="pb-2 px-1 text-center w-24" rowspan="2"><?= __('services.admin.domain.col_discount') ?></th>
                    ...
                    <tr class="text-[10px] text-zinc-300 dark:text-zinc-600 border-b border-zinc-100 dark:border-zinc-700">
                        ...
                        <th class="pb-1 px-1 text-center"><input type="number" id="vipMultiplier" placeholder="<?= htmlspecialchars(__('services.admin.domain.placeholder_multiplier'), ENT_QUOTES) ?>" step="0.1" min="0" class="w-full px-1 py-0.5 text-[10px] text-center border border-blue-300 dark:border-blue-700 rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold" oninput="applyMultiplier('vip', this.value)" title="<?= htmlspecialchars(__('services.admin.domain.tooltip_multiplier'), ENT_QUOTES) ?>"></th>
                        <th class="pb-1 px-1 text-center"><input type="number" id="normalMultiplier" placeholder="<?= htmlspecialchars(__('services.admin.domain.placeholder_multiplier'), ENT_QUOTES) ?>" step="0.1" min="0" class="w-full px-1 py-0.5 text-[10px] text-center border border-blue-300 dark:border-blue-700 rounded bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400 font-bold" oninput="applyMultiplier('normal', this.value)" title="<?= htmlspecialchars(__('services.admin.domain.tooltip_multiplier'), ENT_QUOTES) ?>"></th>
                    </tr>
                </thead>
                <tbody id="domainRows">
        ...
        <button type="button" onclick="addDomainRow()" class="text-xs text-blue-600 dark:text-blue-400 hover:underline"><?= __('services.admin.domain.add_tld') ?></button>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.domain.section_namesilo_api') ?>
        </h3>
    </div>
    <div class="divide-y divide-zinc-100 dark:divide-zinc-700">
        <div class="flex items-center px-6 py-4">
            <label class="w-44 shrink-0 text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('services.admin.domain.label_api_key') ?></label>
            <div class="flex-1">
                ...
                <p class="text-xs text-zinc-400 mt-1"><?= __('services.admin.domain.api_key_hint') ?> <a href="https://www.namesilo.com/account/api-manager" target="_blank" class="text-blue-500 hover:underline"><?= __('services.admin.domain.api_manager_link') ?></a></p>
            </div>
        </div>
        <div class="flex items-center px-6 py-4">
            <label class="w-44 shrink-0 text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('services.admin.domain.label_sandbox') ?></label>
            <div class="flex-1">
                ...
                <p class="text-xs text-zinc-400 mt-1"><?= __('services.admin.domain.sandbox_hint') ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="flex justify-end mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-700">
    <button type="button" onclick="saveDomainSettings()" class="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
        <?= __('services.admin.domain.save_button') ?>
    </button>
</div>
