...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.general.section_title') ?>
        </h3>
    </div>
    <div class="divide-y divide-zinc-100 dark:divide-zinc-700">
        <!-- 표시 통화 -->
        <div class="flex items-center px-6 py-4">
            <label class="w-44 shrink-0 text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('services.admin.general.display_currency') ?></label>
            <div class="flex-1">
                ...
                <p class="text-xs text-zinc-400 mt-1"><?= __('services.admin.general.display_currency_hint') ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700 flex items-center justify-between">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.general.exchange_rates') ?>
        </h3>
        <button type="button" onclick="fetchExchangeRates()" id="btnFetchRates" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 hover:bg-blue-100 dark:hover:bg-blue-900/50 rounded-lg transition">
            ...
            <?= __('services.admin.general.fetch_rates') ?>
        </button>
    </div>
    <div class="divide-y divide-zinc-100 dark:divide-zinc-700">
        <!-- 현재 환율 (4통화 한눈에) -->
        <div class="px-6 py-4">
            ...
            <p class="text-xs text-zinc-400 mt-2" id="rateStatus">
                <?php if ($_rateDate): ?>
                    <?= htmlspecialchars($_rateDate) ?> · <a href="https://frankfurter.dev" target="_blank" class="text-blue-500 hover:underline">Frankfurter API</a>
                <?php else: ?>
                    <?= __('services.admin.general.no_rates') ?>
                <?php endif; ?>
            </p>
        </div>
        <!-- 자동 업데이트 -->
        <div class="flex items-center px-6 py-4">
            <label class="w-44 shrink-0 text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('services.admin.general.auto_update') ?></label>
            <div class="flex-1">
                <select id="svcExchangeAuto" class="<?= $_sel ?>">
                    <?php
                    $autoOptions = [
                        'manual' => __('services.admin.general.auto_manual'),
                        'daily'  => __('services.admin.general.auto_daily'),
                        'weekly' => __('services.admin.general.auto_weekly'),
                    ];
                ...
                <p class="text-xs text-zinc-400 mt-1"><?= __('services.admin.general.auto_update_hint') ?></p>
            </div>
        </div>
    </div>
</div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border dark:border-zinc-700 overflow-hidden mb-6">
    <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
        <h3 class="text-sm font-bold text-zinc-900 dark:text-white flex items-center gap-2">
            ...
            <?= __('services.admin.general.rounding_unit') ?>
        </h3>
    </div>
    <div class="px-6 py-4">
        ...
        <p class="text-xs text-zinc-400 mt-2"><?= __('services.admin.general.rounding_hint') ?></p>
    </div>
</div>

<!-- 저장 버튼 (탭 전용) -->
<div class="flex justify-end mt-6 pt-4 border-t border-zinc-200 dark:border-zinc-700">
    <button type="button" onclick="saveGeneralSettings()" class="inline-flex items-center gap-2 px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium transition">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 13l4 4L19 7"/></svg>
        <?= __('services.admin.general.save_button') ?>
    </button>
</div>
