...
switch ($action) {
    ...
    case 'project_send_quote': {
        ...
        try {
            ...
            // 이전 sent (미수락) 견적은 superseded 로
            $pdo->prepare("UPDATE {$prefix}custom_quotes SET status = 'superseded' WHERE project_id = ? AND status = 'sent' AND id != ?")
                ->execute([$projectId, $quoteId]);
