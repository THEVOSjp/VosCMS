...
CREATE TABLE IF NOT EXISTS `rzx_shops` (
    ...
    `business_license` VARCHAR(500) DEFAULT NULL COMMENT '사업자등록증 파일 경로 (관리자만 열람)',
    ...
    `seat_count` SMALLINT UNSIGNED DEFAULT NULL COMMENT '좌석/시술대 수',
    `opening_status` VARCHAR(20) DEFAULT 'opened' COMMENT 'planned=개업예정, opened=개업완료',
    `opened_at` DATE DEFAULT NULL COMMENT '개업 시기',
...
CREATE TABLE IF NOT EXISTS `rzx_shop_events` (
    ...
    `end_date` DATETIME DEFAULT NULL,
    `is_active` TINYINT(1) DEFAULT 1,
    `payment_status` VARCHAR(20) DEFAULT 'pending' COMMENT 'pending, paid, expired',
    `payment_amount` INT DEFAULT 10000 COMMENT '결제 금액(엔)',
    `payment_id` VARCHAR(100) DEFAULT NULL COMMENT '외부 결제 ID',
    `paid_at` DATETIME DEFAULT NULL,
    `duration_days` INT DEFAULT 7 COMMENT '이벤트 기간(7~30일)',
    `view_count` INT DEFAULT 0,
    `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
    KEY `idx_shop` (`shop_id`),
    KEY `idx_active_date` (`is_active`, `start_date`, `end_date`),
...
CREATE TABLE IF NOT EXISTS `rzx_shop_inquiries` (
    ...
    `attachments` JSON DEFAULT NULL COMMENT '첨부파일 [{name, path, type, size}]',
