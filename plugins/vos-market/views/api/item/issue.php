<?php
/**
 * POST /api/market/item/issue
 * Body: { vos_key, domain, slug, type(issue|qna), title, body, author_name }
 *
 * VosCMS 라이선스 키 + 도메인으로 발신자 검증.
 * 구매자(mkt_licenses 매칭)면 is_verified=1.
 */
...
try {
    ...
    $slug    = trim($body['slug']        ?? '');
    $type    = trim($body['type']        ?? 'issue');
    $title   = trim($body['title']       ?? '');
    $bodyTxt = trim($body['body']        ?? '');
    $name    = trim($body['author_name'] ?? '');

    if (!in_array($type, ['issue','qna'], true)) $type = 'issue';
    if (!$vosKey || !$domain || !$slug || $title === '') {
        http_response_code(400);
        echo json_encode(['ok'=>false,'msg'=>'vos_key, domain, slug, title 필수']); exit;
    }
    ...
    $ins = $pdo->prepare(
        "INSERT INTO {$pfx}mkt_issues (item_id, type, title, body, author_name, author_domain, is_verified, status)
         VALUES (?, ?, ?, ?, ?, ?, ?, 'open')"
    );
    $ins->execute([$itemId, $type, $title, $bodyTxt ?: null, $name ?: null, $domain, $isVerified]);
