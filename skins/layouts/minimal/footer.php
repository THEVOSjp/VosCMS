    ...
    <!-- 메뉴 오버레이 (클릭 시 닫기) -->
    <div id="mobileMenuOverlay" class="hidden lg:hidden fixed inset-0 z-30" onclick="closeMobileMenu()"></div>

    <!-- 드롭업 메뉴 (하단 메뉴바 위) -->
    <div id="mobileMenuPanel" class="hidden lg:hidden fixed bottom-14 left-0 right-0 z-40 safe-area-bottom">
        <div class="mx-3 mb-2 bg-white dark:bg-zinc-800 rounded-2xl shadow-2xl border border-zinc-200 dark:border-zinc-700 overflow-hidden">
            <!-- 메뉴 목록 (하단 바 아이콘과 동일 스타일) -->
            <?php
            // 메뉴 아이콘 매핑
            $_menuIcons = [
            ...
            ?>
            <div class="flex flex-wrap px-2 pt-3 pb-1">
                <a href="<?= $baseUrl ?>/" class="flex flex-col items-center justify-center <?= $_colW ?> py-2 text-zinc-600 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400 transition">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><?= $_menuIcons['home'] ?></svg>
                    <span class="text-[10px]"><?= __('common.nav.home') ?? '홈' ?></span>
                </a>
                ...
                ?>
                <a href="<?= htmlspecialchars($__href) ?>" class="flex flex-col items-center justify-center <?= $_colW ?> py-2 text-zinc-600 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400 transition">
                    <svg class="w-5 h-5 mb-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><?= $__icon ?></svg>
                    <span class="text-[10px]"><?= htmlspecialchars($__mi['title']) ?></span>
                </a>
                <?php endforeach; ?>
            </div>
            <!-- 구분선 -->
