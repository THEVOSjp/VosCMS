<?php
/**
 * RezlyX Skin Component - Terms Agreement
 *
 * 약관 동의 컴포넌트 (재사용 가능)
 *
 * 사용 가능한 변수:
 * - $termsSettings: 약관 설정 배열 (두 가지 형식 지원)
 *   형식 1 (원시): member_term_1_title, member_term_1_content, member_term_1_consent 등
 *   형식 2 (파싱됨): [['id' => 1, 'title' => '...', 'content' => '...', 'required' => true], ...]
 * - $translations: 번역 데이터
 * - $errors: 폼 에러 메시지 배열 (약관 관련)
 */

// 약관 데이터 파싱 (두 가지 형식 지원)
$terms = [];

// 이미 파싱된 배열 형식인지 확인 (AuthController에서 전달된 경우)
if (is_array($termsSettings) && !empty($termsSettings) && isset($termsSettings[0]['id'])) {
    // 이미 파싱된 형식
    $terms = $termsSettings;
} else {
    // 원시 설정 형식 파싱 (다국어 지원)
    $currentLocale = function_exists('current_locale') ? current_locale() : 'ko';
    for ($i = 1; $i <= 5; $i++) {
        $consent = $termsSettings["member_term_{$i}_consent"] ?? 'disabled';

        // 비활성화된 약관은 건너뛰기
        if ($consent === 'disabled') {
            continue;
        }

        // db_trans()를 사용하여 번역 조회, 없으면 기본 설정값 사용
        $defaultTitle = $termsSettings["member_term_{$i}_title"] ?? '';
        $defaultContent = $termsSettings["member_term_{$i}_content"] ?? '';

        if (function_exists('db_trans')) {
            $title = db_trans("term.{$i}.title", $currentLocale, $defaultTitle);
            $content = db_trans("term.{$i}.content", $currentLocale, $defaultContent);
        } else {
            $title = $defaultTitle;
            $content = $defaultContent;
        }

        if (empty($title)) {
            continue;
        }

        $terms[] = [
            'id' => $i,
            'title' => $title,
            'content' => $content,
            'consent' => $consent, // 'required' or 'optional'
            'required' => ($consent === 'required'),
        ];
    }
}

// 약관이 없으면 표시하지 않음
if (empty($terms)) {
    return;
}

$hasRequiredTerms = false;
foreach ($terms as $term) {
    if ($term['required']) {
        $hasRequiredTerms = true;
        break;
    }
}
?>

<!-- Terms Agreement Section -->
<div class="terms-agreement-section">
    <div class="mb-4">
        <h3 class="text-base font-semibold text-gray-900 dark:text-white mb-2">
            <?= __('auth.terms.title') ?>
        </h3>
        <p class="text-sm text-gray-600 dark:text-zinc-400">
            <?= __('auth.terms.subtitle') ?>
        </p>
    </div>

    <!-- 에러 메시지 -->
    <?php if (!empty($errors['terms'])): ?>
    <div class="mb-4 p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
        <p class="text-sm text-red-700 dark:text-red-300"><?= htmlspecialchars($errors['terms']) ?></p>
    </div>
    <?php endif; ?>

    <!-- 전체 동의 체크박스 -->
    <?php if (count($terms) > 1): ?>
    <div class="mb-4 p-4 bg-gray-50 dark:bg-zinc-800 rounded-lg border border-gray-200 dark:border-zinc-700">
        <label class="flex items-center cursor-pointer group">
            <input type="checkbox" id="agreeAllTerms" name="agree_all"
                   class="w-5 h-5 text-blue-600 border-gray-300 dark:border-zinc-600 rounded focus:ring-blue-500"
                   onchange="toggleAllTerms(this.checked)">
            <span class="ml-3 text-base font-medium text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">
                <?= __('auth.terms.agree_all') ?>
            </span>
        </label>
        <?php if ($hasRequiredTerms): ?>
        <p class="mt-1 ml-8 text-xs text-gray-500 dark:text-zinc-400">
            <?= __('auth.terms.required_note') ?>
        </p>
        <?php endif; ?>
    </div>
    <?php endif; ?>

    <!-- 개별 약관 목록 -->
    <div class="space-y-3">
        <?php foreach ($terms as $index => $term): ?>
        <div class="border border-gray-200 dark:border-zinc-700 rounded-lg overflow-hidden">
            <!-- 약관 헤더 (접히는 영역) -->
            <div class="p-4 bg-white dark:bg-zinc-800">
                <div class="flex items-start">
                    <input type="checkbox"
                           id="term_<?= $term['id'] ?>"
                           name="terms[<?= $term['id'] ?>]"
                           value="1"
                           class="term-checkbox w-4 h-4 mt-0.5 text-blue-600 border-gray-300 dark:border-zinc-600 rounded focus:ring-blue-500"
                           <?= $term['required'] ? 'required' : '' ?>
                           data-required="<?= $term['required'] ? 'true' : 'false' ?>"
                           onchange="checkAllTermsStatus()">
                    <div class="ml-3 flex-1">
                        <label for="term_<?= $term['id'] ?>" class="flex items-center cursor-pointer">
                            <span class="text-sm font-medium text-gray-900 dark:text-white">
                                <?= htmlspecialchars($term['title']) ?>
                            </span>
                            <?php if ($term['required']): ?>
                            <span class="ml-1 text-red-500 dark:text-red-400" title="<?= __('auth.terms.required') ?>">*</span>
                            <span class="ml-2 px-2 py-0.5 text-xs bg-red-100 dark:bg-red-900/40 text-red-700 dark:text-red-300 rounded">
                                <?= __('auth.terms.required') ?>
                            </span>
                            <?php else: ?>
                            <span class="ml-2 px-2 py-0.5 text-xs bg-gray-100 dark:bg-zinc-700 text-gray-600 dark:text-zinc-400 rounded">
                                <?= __('auth.terms.optional') ?>
                            </span>
                            <?php endif; ?>
                            <?php if (!empty($term['isFallback'])): ?>
                            <span class="ml-2 px-2 py-0.5 text-xs bg-amber-100 dark:bg-amber-900/40 text-amber-700 dark:text-amber-300 rounded animate-pulse">
                                ⚠ <?= __('auth.terms.translation_pending') ?>
                            </span>
                            <?php endif; ?>
                        </label>
                    </div>
                    <?php if (!empty($term['page_slug'])): ?>
                    <!-- 시스템 페이지: 새 탭 링크로 전문 보기 -->
                    <a href="/<?= htmlspecialchars($term['page_slug']) ?>" target="_blank" rel="noopener"
                       class="ml-2 inline-flex items-center gap-1 text-xs text-blue-600 dark:text-blue-400 hover:underline whitespace-nowrap">
                        <?= __('auth.terms.view_full') ?>
                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"/>
                        </svg>
                    </a>
                    <?php elseif (!empty($term['content'])): ?>
                    <button type="button"
                            onclick="toggleTermContent('<?= $term['id'] ?>')"
                            class="ml-2 p-1 text-gray-400 hover:text-gray-600 dark:text-zinc-500 dark:hover:text-zinc-300 transition-colors">
                        <svg id="termIcon_<?= $term['id'] ?>" class="w-5 h-5 transform transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"/>
                        </svg>
                    </button>
                    <?php endif; ?>
                </div>
            </div>

            <!-- 약관 내용 (page_slug 없는 커스텀 약관만 인라인 표시) -->
            <?php if (empty($term['page_slug']) && !empty($term['content'])): ?>
            <div id="termContent_<?= $term['id'] ?>" class="hidden border-t border-gray-200 dark:border-zinc-700">
                <div class="p-4 bg-gray-50 dark:bg-zinc-900/50 max-h-48 overflow-y-auto text-sm text-gray-700 dark:text-zinc-300 terms-content">
                    <?= $term['content'] ?>
                </div>
            </div>
            <?php endif; ?>
        </div>
        <?php endforeach; ?>
    </div>
</div>

<style>
/* 약관 내용 Typography 스타일 - Tailwind Typography 플러그인 대체 */
.terms-content {
    font-family: 'Pretendard', -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif;
    line-height: 1.7;
    word-break: keep-all;
}
.terms-content h1 {
    font-size: 1.5em;
    font-weight: 700;
    margin: 1em 0 0.5em 0;
    color: #1f2937;
}
.terms-content h2 {
    font-size: 1.25em;
    font-weight: 600;
    margin: 1em 0 0.5em 0;
    color: #374151;
}
.terms-content h3 {
    font-size: 1.125em;
    font-weight: 600;
    margin: 0.8em 0 0.4em 0;
    color: #4b5563;
}
.terms-content p {
    margin: 0.5em 0;
    line-height: 1.75;
}
.terms-content ul, .terms-content ol {
    margin: 0.5em 0;
    padding-left: 1.5em;
}
.terms-content ul {
    list-style-type: disc;
}
.terms-content ol {
    list-style-type: decimal;
}
.terms-content li {
    margin: 0.25em 0;
}
.terms-content blockquote {
    border-left: 3px solid #d1d5db;
    padding-left: 1em;
    margin: 0.5em 0;
    color: #6b7280;
    font-style: italic;
}
.terms-content strong, .terms-content b {
    font-weight: 600;
}
.terms-content a {
    color: #2563eb;
    text-decoration: underline;
}
.terms-content a:hover {
    color: #1d4ed8;
}
/* 다크 모드 스타일 */
.dark .terms-content h1 {
    color: #f3f4f6;
}
.dark .terms-content h2 {
    color: #e5e7eb;
}
.dark .terms-content h3 {
    color: #d1d5db;
}
.dark .terms-content blockquote {
    border-left-color: #4b5563;
    color: #9ca3af;
}
.dark .terms-content a {
    color: #60a5fa;
}
.dark .terms-content a:hover {
    color: #93c5fd;
}
</style>

<script>
// 전체 동의 토글
function toggleAllTerms(checked) {
    var checkboxes = document.querySelectorAll('.term-checkbox');
    checkboxes.forEach(function(checkbox) {
        checkbox.checked = checked;
    });
    console.log('All terms toggled:', checked);
}

// 개별 체크박스 상태 확인하여 전체 동의 체크박스 업데이트
function checkAllTermsStatus() {
    var checkboxes = document.querySelectorAll('.term-checkbox');
    var allChecked = true;
    checkboxes.forEach(function(checkbox) {
        if (!checkbox.checked) {
            allChecked = false;
        }
    });

    var agreeAllCheckbox = document.getElementById('agreeAllTerms');
    if (agreeAllCheckbox) {
        agreeAllCheckbox.checked = allChecked;
    }
    console.log('Terms status checked, all agreed:', allChecked);
}

// 약관 내용 토글
function toggleTermContent(termId) {
    var content = document.getElementById('termContent_' + termId);
    var icon = document.getElementById('termIcon_' + termId);

    if (content && icon) {
        if (content.classList.contains('hidden')) {
            content.classList.remove('hidden');
            icon.classList.add('rotate-180');
        } else {
            content.classList.add('hidden');
            icon.classList.remove('rotate-180');
        }
        console.log('Term content toggled:', termId);
    }
}

// 폼 제출 전 필수 약관 확인
function validateTermsAgreement() {
    var requiredTerms = document.querySelectorAll('.term-checkbox[data-required="true"]');
    var allRequiredChecked = true;

    requiredTerms.forEach(function(checkbox) {
        if (!checkbox.checked) {
            allRequiredChecked = false;
            checkbox.closest('.border').classList.add('border-red-500', 'dark:border-red-500');
        } else {
            checkbox.closest('.border').classList.remove('border-red-500', 'dark:border-red-500');
        }
    });

    if (!allRequiredChecked) {
        console.log('Required terms not agreed');
        alert('<?= __('auth.terms.required_alert') ?>');
    }

    return allRequiredChecked;
}
</script>
