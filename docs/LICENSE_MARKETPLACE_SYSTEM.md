...
| 서비스 | 플랜 | 사양 | 월 비용 | 특징 |
|--------|------|------|---------|------|
| **ConoHa VPS** | 2GB | 3 vCPU, 2GB RAM, 100GB SSD | ¥1,848 | 일본 데이터센터, 시간제 과금, API 제공 |
| **Xserver VPS** | 2GB | 3 vCPU, 2GB RAM, 50GB NVMe | ¥1,150 | Xserver 계열, 안정적, 저렴 |
| **Vultr** | Regular 2GB | 1 vCPU, 2GB RAM, 50GB SSD | $12 (~¥1,800) | 도쿄 리전, 글로벌, 유연한 스케일 |
| **DigitalOcean** | Basic 2GB | 1 vCPU, 2GB RAM, 50GB SSD | $12 (~¥1,800) | 싱가포르 리전, 관리 UI 우수 |
| **AWS Lightsail** | 2GB | 2 vCPU, 2GB RAM, 60GB SSD | $12 (~¥1,800) | 도쿄 리전, AWS 생태계 연동 |

#### 추천: **ConoHa VPS 2GB** 또는 **Xserver VPS 2GB**

**선정 이유:**
- 일본 내 데이터센터 → 한일 고객 모두 저지연
- root 권한 → ionCube, PHP, Nginx 자유 설치
- SSD/NVMe → 라이선스 API 응답 속도
- 월 ¥1,150~1,848 → 사업 초기 비용 부담 적음
- 향후 스케일업 간단 (2GB → 4GB → 8GB)
...
voscms.com (VPS)
  ...
  │
  ├── voscms.com/                   ← 메인 사이트 (VosCMS 소개)
  ├── voscms.com/api/license/       ← 라이선스 API
  ├── voscms.com/api/marketplace/   ← 마켓플레이스 API (카탈로그)
  ├── voscms.com/marketplace/       ← 프론트엔드 마켓플레이스 (공개)
  ├── voscms.com/developer/         ← 개발자 대시보드
  └── voscms.com/admin/             ← 본사 관리 (라이선스+심사+정산)
```

#### VPS 이전 절차

VPS 구매 후 SSH 접속 정보(IP, 포트, 키)만 제공하면 Claude가 전체 세팅:
...
VPS 확보 전까지 **vos.21ces.com에서 라이선스 서버를 개발하고 테스트**한다.

```
[현재] vos.21ces.com (PHP 8.3 + ionCube v15.5)
  ├── VosCMS 본체 개발/테스트
  ├── 라이선스 서버 API 개발/테스트    ← 여기서 개발
  └── 마켓플레이스 플러그인 테스트

[향후] voscms.com VPS로 라이선스 서버 이전
  └── vos.21ces.com의 API + DB를 마이그레이션
```
