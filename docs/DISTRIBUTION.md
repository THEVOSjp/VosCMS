...
| **번들 플러그인 1개** | `vos-autoinstall` (자동 설치 / 마켓 다운로드) — `vos-salon` 은 2.4.3 부터 별도 판매로 분리되어 dist 미포함 |
...
| `vos-salon` | 살롱 비즈니스용 — 마켓에서 별도 판매 (2.4.3 부터 dist 분리) |
...
| `admin/` | offline.html 잉여 디렉토리 — 빈 디렉토리라 nginx 디렉토리 인덱싱으로 403 발생, root `/offline.html` 만 sw.js 가 참조 (2.4.3 부터) |

### 3.6 제외 — 예약 시스템 마이그레이션 (2.4.3 부터)

호스팅 고객은 예약(reservation) 시스템 미설치 → 관련 SQL 마이그레이션 제외:

| SQL 파일 | 이유 |
|---------|------|
| `001_create_reservation_tables.sql` | 예약 테이블 생성 |
| `011_seed_service_translations.sql`, `012_seed_service_translations_p2.sql` | 예약 서비스 다국어 시드 |
| `013_add_source_to_reservations.sql`, `014_add_payment_status_to_reservations.sql` | 예약 컬럼 추가 |
| `015_create_reservation_core.sql` | 예약 코어 테이블 (`001_create_base_tables.sql` 에서 분리) |
| `016_alter_reservation_services.sql` | 예약 서비스 변경 |
| `024_reservations_bundle_columns.sql` | 예약 묶음 컬럼 |

**근거**: 2.4.3 부터 `001_create_base_tables.sql` (15 테이블) 을 핵심 5개 (`admins`, `users`, `member_grades`, `settings`, `migrations`) 만 남기고 나머지 9개 reservation 테이블은 신규 `015_create_reservation_core.sql` 로 분리. 호스팅 dist 빌드 시 015 exclude → `rzx_reservations` 테이블 미생성 → admin 대시보드의 예약 카렌다 자동 미표시 (`$hasReservations = false`).
