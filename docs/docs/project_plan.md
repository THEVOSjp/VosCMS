...
### 2026-03-23: 모바일 메뉴바 + 레이아웃 적용 + 회원 페이지 통합 (v1.16.0)

- [x] 모바일 하단 메뉴바 (스태프/예약/홈/로그인/회원가입/더보기)
- [x] 레이아웃/스킨 설정 실제 렌더링 적용
- [x] 회원 페이지 4개 기본 레이아웃 통합
- [x] 스킨 `<main>` 추출로 중복 헤더/JS 제거
- [x] 메뉴 로더 include 문제 수정
- [x] deprecated meta 태그 수정 (8개 파일)
- [x] 다국어 nav.staff/nav.more 13개 언어 추가

### 2026-03-22: 레이아웃 상세 설정 + SkinConfigRenderer 탭 지원 (v1.15.3)

- [x] 상세 설정 패널 (SkinConfigRenderer 기반 AJAX 폼)
- [x] layout.json 메뉴 설정 (sitemaps 드롭다운)
- [x] layout.json 확장 (menus + vars 14개 + 13개 언어)
- [x] SkinConfigRenderer 탭 UI 자동 생성 (`tab` 속성)
- [x] SkinConfigRenderer 리팩토링 (renderVar 통합, 코드 중복 제거)
- [x] 미리보기 연동 (사용 안 함 → no_layout)
- [ ] 레이아웃/스킨 설정값 실제 렌더링 적용

### 2026-03-22: 레이아웃 관리 페이지 구현 (v1.15.2)

- [x] 레이아웃 관리 3단 구조 (미리보기+메뉴 / 스킨 목록 / 상세 설정)
- [x] 레이아웃/페이지/게시판/회원 4개 카테고리 스킨 선택 + DB 저장
- [x] "디자인 관리" → "레이아웃 관리" 명칭 변경 (13개 언어)
- [ ] 상세 설정 패널 구현 (SkinConfigRenderer 활용, 다음 세션)
- [ ] 복사본 생성/삭제 기능 구현

### 2026-03-22: 사이트 관리 15개 페이지 _head.php 통일 (v1.15.1)

- [x] 게시판 4개 (boards, create, edit, trash) _head.php 적용
- [x] 메뉴/디자인/페이지/위젯 11개 페이지 _head.php 적용
- [x] embed 모드 페이지 비embed 분기 처리
- [x] 전체 관리자 페이지 헤더/레이아웃 통일 완료

### 2026-03-22: 시스템 위젯 + 위젯 빌더 확장 + UI 통합 (v1.15.0)

- [x] 시스템 위젯 4개 (staff, booking, lookup, cta001)
- [x] 시스템 페이지 3개 (staff, booking, lookup) DB + 라우팅
- [x] 위젯 기반 페이지 렌더링 (위젯 배치 → 위젯, 미배치 → 기존 폴백)
- [x] 위젯 자동 DB 등록 (syncToDatabase)
- [x] 위젯 빌더 slug 동적 지원 (home 고정 → 모든 페이지)
- [x] 관리자 헤더 통합 (pageSubTitle/pageSubDesc)
- [x] 스태프 10개 + 회원 4개 페이지 _head.php 통일
- [x] 페이지 목록 루프 리팩토링 (기어/수정/미리보기 아이콘)

### 2026-03-22: 스킨 시스템 강화 + UI 통합 + 보안 (v1.14.1)

- [x] 페이지 스킨 탭 (기본정보 + 확장 변수 + 파일 업로드)
- [x] depends_on 시스템 (skin.json 선언적 필드 의존 관계)
- [x] 제목 배경 시스템 (이미지/동영상/오버레이, 전 타입 적용)
- [x] 카드형 레이아웃/스킨 선택 UI
- [x] SEO 시스템 + OG Image 업로드
- [x] 공통 결과 모달 (13개 언어)
- [x] 서비스 카드 뷰 + 드래그 앤 드롭 순서
- [x] 예약 스태프 배정/지명 + 예약 경로 UI
- [x] 보안 헤더 (X-Frame-Options, CSP)
- [x] 키오스크/번들 페이지 _head.php 통일
- [x] 파비콘 DB 설정 적용
- [x] PAGE_SYSTEM.md, PRD_OrderSystem.md, PRD_KioskSystem.md 문서

### 2026-03-21: 페이지 시스템 + 메뉴 시스템 마무리 (v1.13.0)

- [x] 메뉴 항목 추가 시 문서/위젯/외부 페이지 자동 생성
- [x] 동적 페이지 렌더러 (문서→HTML, 위젯→WidgetRenderer, 외부→iframe/include)
- [x] 페이지 환경 설정 (기본/레이아웃·스킨/SEO 3탭)
- [x] 페이지 콘텐츠 편집 (언어별 편집, slug/타입 변경, 삭제)
- [x] 페이지 관리 동적 목록 (타입별 아이콘·배지)
- [x] 메뉴 바로가기 URL 수정 + 편집 UI 통일
- [x] 메뉴 13개 언어 번역 완성
- [x] 메뉴 삭제 시 연결 페이지·번역 함께 삭제
- [x] DB page_type 컬럼 + 마이그레이션

### 2026-03-20: 스킨 설정 시스템 + 목록 4스타일 + 제목 배경 (v1.12.0)

- [x] 스킨 설정 시스템 완성 (skin.json 섹션, radio/video 타입, 파일 업로드/삭제)
- [x] 스킨 기본정보 (제작자/URL/이메일/날짜/버전/썸네일)
- [x] 제목 배경 이미지/동영상 (높이/오버레이/텍스트색 설정)
- [x] 목록 4스타일 (목록/웹진/갤러리/카드) + 스킨 파일 오버라이드
- [x] 갤러리형 풀이미지+호버 애니메이션, 공지 상단 분리
- [x] 스킨 설정 실적용 (색상/모서리/열수/분류표시/링크게시판 등)
- [x] skin.json 기본값 자동 병합, skin_config 저장 버그 수정

### 2026-03-20: URL 자동 링크 + OG 카드 프리뷰 (v1.11.0)

- [x] URL 자동 링크 (에디터 입력/붙여넣기 + 본문 표시)
- [x] OG 링크 프리뷰 카드 (SNS 배너 형식, 가운데 정렬)
- [x] OgFetcher 모듈 + OG API 엔드포인트
- [x] board-content.css 전역 공통 스타일 (에디터+본문)
- [x] Tailwind preflight 이미지 정렬 문제 해결
...
- [x] **예약하기 페이지 스킨 설정 적용** (v1.18.2)
  - booking.php 위젯 렌더링 경로에 page.php와 동일한 스킨 적용 로직 추가
  - 제목 배경(이미지/동영상), content_width, custom_css, breadcrumb 등 적용
  - 설정/편집 아이콘을 페이지 레벨 타이틀에 통합 (위젯 내 중복 제거)
  - `_page-title-bg.php` partial 공용 사용

- [x] **예약 상세 모듈화 + 번들 저장 개선** (v1.19.0)
  - 예약 상세 UI를 공통 모듈로 분리 (components/reservation-detail-*.php)
  - booking/detail.php, mypage/reservation-detail.php 양쪽에서 재사용
  - 번들 정보 저장 개선: reservations.bundle_id/bundle_price 컬럼 추가
  - 서비스 원래 가격 유지, 번들 가격은 별도 저장
  - 다국어 적용 (날짜, 전화번호, 스태프 미배정 등)
- [x] **결제 모듈 Phase 1 시작** (v1.19.0)
  - 관리자 설정 > 일반 > 온라인 결제 설정 UI 추가
  - PG사 선택 (Stripe/토스/PAY.JP/포트원), API 키 입력, 테스트 모드
  - PAYMENT_MODULE.md 개발 문서 작성
  - 13개 언어 번역 적용

- [x] **결제 모듈 Phase 1~2 완료 + Stripe 연동** (v1.20.0)
  - 결제 코어: PaymentGatewayInterface, PaymentManager, PaymentService, DTO
  - Stripe Gateway: Embedded Checkout, 결제 승인, 환불, Webhook
  - DB: rzx_payments, rzx_refunds 테이블, reservations.payment_id 추가
  - 결제 플로우: 예약 생성(pending) → 결제 → 예약 확정(confirmed)
  - 예약금 설정 연동: 활성화 시 설정된 예약금만 결제
  - 결제 취소 시 예약 정보 유지 + 재결제 가능
  - booking.php 번들 저장 개선 (bundle_id, bundle_price)
  - 예약 슬롯 로직 개선: 미지정 예약 시 전체 스태프 인원 기준
  - 결제 화면 다국어 (Stripe 로케일, 서비스명, 번들 표시명)
  - 관리자 결제 설정 UI 고도화 (PG사 안내 카드, 다국어)

- [x] **환불 정책 + 관리자 예약 상세 개선** (v1.21.0)
  - 환불 정책 설정 UI (시간/일 단위, 전액/부분 3단계/환불불가)
  - 예약 취소 시 Stripe 환불 API 자동 호출 + rzx_refunds 기록
  - 결제 완료/예약 상세에 환불 정책 안내 표시
  - 관리자 예약 상세: 노쇼 버튼 추가, JS alert → 커스텀 모달
  - 관리자 예약 상세: 일시 수정, 연락처 수정 모달
  - 관리자 예약 상세: 예약번호 타이틀 통합, 예약금 표시 개선
  - 관리자 예약 상세: 고객 아바타 확대, 국제전화 포맷
  - 결제 정보 영수증/인쇄 버튼 (Stripe receipt_url 연동)
  - 게시판 배너 제목 중앙 정렬, 콘텐츠 너비 설정 추가

- [x] **관리자 예약 상세 서비스/결제 개선** (v1.22.0)
  - 서비스 추가 카드형 UI (4열 그리드 + 번들 3열)
  - 번들 상단 표시 + 번들 삭제(포함 서비스 일괄 삭제)
  - 번들 포함 서비스 개별 삭제 방지
  - 서비스 전체 삭제 시 예약 유지
  - 결제 정보 레이아웃 개선 (번들 소계/적용가/추가서비스/지명료/예약금 분리)
  - 요일 다국어 표시 수정

- [x] **POS + 관리자 상세 다국어 전면 적용** (v1.23.0)
  - POS 대기 카드: 스태프명/서비스명 다국어 (name_i18n + translations)
  - POS 서비스 내역 모달: 고객정보/성별/등급명/방문통계/메모 다국어
  - POS 서비스 추가: 번들 목록 추가 + 서비스명 다국어
  - POS 스태프 배정: name_i18n 다국어 적용
  - 관리자 상세: 스태프명 name_i18n, 서비스 설명 다국어, 시간 단위 다국어
  - 관리자 상세: 현금/카드 결제 버튼 + 적립금 사용 + Stripe 모달
  - 관리자 상세: 서비스 삭제 확인 모달
  - 알려진 이슈: POS 결제 합계 계산 (고객 그룹핑 시 다른 예약 paid_amount 합산)

- [x] **ReservationHelper 공용 모듈 + POS 예약번호 기준 카드** (v1.24.0)
  - ReservationHelper 클래스: create(), appendServices(), resolveServices(), saveServicesPublic()
  - 7곳 예약 저장 로직 → 공용 헬퍼 사용으로 통일
  - 번들 저장: service_bundle_items에서 전체 서비스 스냅샷 저장 (문서화)
  - POS 카드: 고객 그룹핑 → 예약번호 기준 (1예약=1카드)
  - POS 서비스 내역: 번들 상품 헤더 + 포함 서비스 목록 + 합계 개선
  - RESERVATION_SYSTEM.md: 번들 스냅샷 저장 규칙 문서화
