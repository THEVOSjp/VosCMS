...
CREATE TABLE IF NOT EXISTS `rzx_users` (
    ...
    `name` VARCHAR(100) NOT NULL,
    `furigana` VARCHAR(100) NULL COMMENT '후리가나 (일본어 읽기)',
    `phone` VARCHAR(20) NULL,
    `profile_image` VARCHAR(255) NULL,
    `birth_date` DATE NULL,
    `gender` ENUM('male', 'female', 'other') NULL,
    `company` VARCHAR(200) NULL COMMENT '회사/소속',
    `blog` VARCHAR(500) NULL COMMENT '블로그/웹사이트',
    `privacy_settings` JSON NULL COMMENT '개인정보 공개 설정',
    `withdraw_reason` TEXT NULL COMMENT '탈퇴 사유',
