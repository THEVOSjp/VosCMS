    ...
    public function confirm(string $sessionId): PaymentResult
    {
        ...
        if ($result->isSuccessful()) {
            ...
            if ($payment && $payment['reservation_id']) {
                // 누적 결제 금액 기준으로 결제 상태 판정
                $resStmt = $this->pdo->prepare("SELECT final_amount, paid_amount FROM {$this->prefix}reservations WHERE id = ?");
                $resStmt->execute([$payment['reservation_id']]);
                $resRow = $resStmt->fetch(\PDO::FETCH_ASSOC);
                $resFinal = (float)($resRow['final_amount'] ?? 0);
                $currentPaid = (float)($resRow['paid_amount'] ?? 0);
                $newPaid = $currentPaid + $result->amount;
                $payStatus = ($newPaid >= $resFinal) ? 'paid' : 'partial';

                $this->pdo->prepare("UPDATE {$this->prefix}reservations SET status = 'confirmed', payment_status = ?, paid_amount = ?, payment_id = ? WHERE id = ?")
                    ->execute([$payStatus, min($newPaid, $resFinal), $payment['id'], $payment['reservation_id']]);
            }
        } else {
            // 실패 기록
