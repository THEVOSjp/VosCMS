...
if (!function_exists('format_phone')) {
    /**
     * 국제 전화번호를 국가별 포맷으로 표시
     * format_phone('+821012345678') → '+82 010-1234-5678'
     * format_phone('+819012345678') → '+81 090-1234-5678'
     */
    function format_phone(?string $raw): string
    {
        if (!$raw) return '';
        // 길이 긴 국가코드부터 매칭 (예: +852 before +8)
        static $codes = ['+1809','+1876','+1868','+1787','+1869','+1242','+1246','+1268','+1441','+1473','+1671','+1758','+1767',
            '+886','+880','+856','+855','+850','+971','+966','+965','+973','+974','+968','+962','+961','+963','+964','+972','+970','+967',
            '+593','+591','+595','+598','+506','+507','+503','+502','+504','+505',
            '+351','+358','+354','+353','+420','+421','+359','+380','+375','+370','+371','+372','+385','+386','+381','+382','+387','+389','+355','+383','+352','+377','+378','+376','+350','+356','+357',
            '+679','+675','+685','+676','+678','+674','+686','+677','+691','+680','+692',
            '+599','+590','+596','+297','+995','+994','+998','+992','+996','+993',
            '+234','+254','+255','+256','+251','+233','+225','+221','+237','+243','+242','+263','+260','+258','+244','+249','+211','+252','+253','+291','+250','+257','+265','+266','+267','+268','+264','+261','+230','+248','+238','+220','+224','+231','+232','+223','+227','+226','+228','+229','+235','+241',
            '+212','+213','+216','+218',
            '+82','+81','+86','+65','+66','+84','+60','+62','+63','+91','+92','+93','+94','+95','+98',
            '+44','+49','+33','+39','+34','+31','+32','+41','+43','+46','+47','+45','+48','+40','+36','+30','+90',
            '+52','+55','+54','+56','+57','+51','+58','+53','+20','+27','+61','+64',
            '+1','+7'];
        $cc = ''; $n = '';
        $digits = preg_replace('/[^\d]/', '', substr($raw, 1)); // +제거 후 숫자
        foreach ($codes as $c) {
            $cDigits = substr($c, 1); // +제거
            if (str_starts_with($digits, $cDigits)) {
                $cc = $c;
                $n = substr($digits, strlen($cDigits));
                break;
            }
        }
        if (!$cc) return $raw;
        ...
        $formatted = match($cc) {
            '+82' => (function() use ($n) { // 한국
                $n = substr($n, 0, 11);
                if (preg_match('/^(01[016789])(\d{3,4})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                if (preg_match('/^(02)(\d{3,4})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                if (preg_match('/^(\d{3})(\d{3,4})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+81' => (function() use ($n) { // 일본
                $n = substr($n, 0, 11);
                if (preg_match('/^(\d{2,4})(\d{2,4})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+1' => (function() use ($n) { // 미국/캐나다
                $n = substr($n, 0, 10);
                if (preg_match('/^(\d{3})(\d{3})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+86' => (function() use ($n) { // 중국
                $n = substr($n, 0, 11);
                if (preg_match('/^(\d{3})(\d{4})(\d{4})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+44' => (function() use ($n) { // 영국
                $n = substr($n, 0, 10);
                if (preg_match('/^(\d{4})(\d{3})(\d{3})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+33' => (function() use ($n) { // 프랑스
                $n = substr($n, 0, 9);
                if (preg_match('/^(\d)(\d{2})(\d{2})(\d{2})(\d{2})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}-{$p[4]}-{$p[5]}";
                return $n;
            })(),
            '+49' => (function() use ($n) { // 독일
                $n = substr($n, 0, 11);
                if (preg_match('/^(\d{3})(\d{4})(\d+)$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                return $n;
            })(),
            '+61' => (function() use ($n) { // 호주
                if (str_starts_with($n, '0')) {
                    if (preg_match('/^(\d{4})(\d{3})(\d{3})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                } else {
                    if (preg_match('/^(\d{3})(\d{3})(\d{3})$/', $n, $p)) return "{$p[1]}-{$p[2]}-{$p[3]}";
                }
                return $n;
            })(),
            default => (function() use ($n) { // 기타: 4자리씩 구분
