...
<body class="bg-gray-50 dark:bg-zinc-900 min-h-screen transition-colors duration-200">
    ...
    <main class="<?= $isEmbed ? 'p-6' : 'max-w-4xl mx-auto px-4 py-12' ?>">
        <div class="bg-white dark:bg-zinc-800 rounded-2xl shadow-lg dark:shadow-zinc-900/50 p-8 md:p-12 transition-colors">
            ...
            <div class="mt-8 pt-6 border-t dark:border-zinc-700">
                <p class="text-sm text-gray-500 dark:text-zinc-400">
                    <?= __('customer.refund_policy.contact', ['name' => htmlspecialchars($siteName)]) ?>
                </p>
            </div>
        </div>
        ...
        <div class="flex justify-between items-center mt-8">
            <a href="<?= $baseUrl ?>/" class="text-blue-600 dark:text-blue-400 hover:underline">&larr; <?= __('customer.refund_policy.back_home') ?></a>
            <a href="<?= $baseUrl ?>/terms" class="text-blue-600 dark:text-blue-400 hover:underline"><?= __('customer.refund_policy.terms_link') ?> &rarr;</a>
        </div>
        <?php endif; ?>
    </main>
