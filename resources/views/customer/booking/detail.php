...
// 결제 상태
$paymentLabels = ['unpaid' => __('booking.payment.unpaid'), 'paid' => __('booking.payment.paid'), 'partial' => __('booking.payment.partial'), 'refunded' => __('booking.payment.refunded')];
$paymentLabel = $paymentLabels[$reservation['payment_status']] ?? $reservation['payment_status'];
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div class="p-6 border-b border-zinc-100 dark:border-zinc-700">
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <p class="text-lg font-semibold text-zinc-800 dark:text-zinc-100">
                            <?= date('Y년 m월 d일 (D)', strtotime($reservation['reservation_date'])) ?>
                        </p>
                ...
                <div class="flex items-start gap-3 shrink-0">
                    <?php
                    // 상태별 인사말: pending/confirmed → before, completed → after
                    $greeting = in_array($reservation['status'], ['completed'])
                    ...
                    <div class="text-center">
                        ...
                        <p class="text-[10px] text-amber-600 dark:text-amber-400"><?= __('booking.detail.designation_fee') ?> <?= fmtPrice($reservation['designation_fee'], $currencySymbol) ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                <?php endif; ?>
        ...
        <div class="px-6 py-4 border-b border-zinc-100 dark:border-zinc-700">
            ...
            <div class="space-y-2">
                ...
                <div class="flex items-center justify-between">
                    <div>
                        <span class="text-sm text-zinc-800 dark:text-zinc-200"><?= htmlspecialchars($svc['service_name'] ?: '서비스') ?></span>
                        <span class="text-xs text-zinc-400 ml-2"><?= (int)$svc['duration'] ?>분</span>
                    </div>
                    <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= fmtPrice($svc['price'], $currencySymbol) ?></span>
                </div>
                <?php endforeach; ?>
        ...
        <div class="px-6 py-4">
            ...
            <div class="space-y-1.5 text-sm">
                <div class="flex justify-between">
                    ...
                    <span class="text-zinc-800 dark:text-zinc-200"><?= fmtPrice($reservation['total_amount'], $currencySymbol) ?></span>
                </div>
                <?php if (($reservation['discount_amount'] ?? 0) > 0): ?>
                <div class="flex justify-between text-green-600 dark:text-green-400">
                    <span><?= __('booking.detail.discount') ?></span>
                    <span>-<?= fmtPrice($reservation['discount_amount'], $currencySymbol) ?></span>
                </div>
                <?php endif; ?>
                <?php if (($reservation['points_used'] ?? 0) > 0): ?>
                <div class="flex justify-between text-green-600 dark:text-green-400">
                    <span><?= __('booking.detail.points_used') ?></span>
                    <span>-<?= fmtPrice($reservation['points_used'], $currencySymbol) ?></span>
                </div>
                <?php endif; ?>
    ...
    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-zinc-200 dark:border-zinc-700 p-6 mb-6">
        <h3 class="text-sm font-medium text-zinc-500 dark:text-zinc-400 mb-3"><?= __('booking.detail.customer_info') ?></h3>
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
            <div>
                <span class="text-zinc-500"><?= __('booking.detail.name') ?></span>
                <p class="font-medium text-zinc-800 dark:text-zinc-200"><?= htmlspecialchars($reservation['customer_name']) ?></p>
            </div>
            <?php if ($reservation['customer_email']): ?>
            <div>
                <span class="text-zinc-500"><?= __('booking.detail.email') ?></span>
                <p class="font-medium text-zinc-800 dark:text-zinc-200"><?= htmlspecialchars($reservation['customer_email']) ?></p>
            </div>
            <?php endif; ?>
            <div>
                <span class="text-zinc-500"><?= __('booking.detail.phone') ?></span>
                <p class="font-medium text-zinc-800 dark:text-zinc-200"><?= htmlspecialchars($reservation['customer_phone']) ?></p>
            </div>
            <div>
                <span class="text-zinc-500"><?= __('booking.detail.source') ?></span>
                <p class="font-medium text-zinc-800 dark:text-zinc-200"><?= $reservation['source'] === 'online' ? 'Online' : ($reservation['source'] === 'pos' ? 'POS' : $reservation['source']) ?></p>
            </div>
        </div>
        <?php if (!empty($reservation['notes'])): ?>
        <div class="mt-3 pt-3 border-t border-zinc-100 dark:border-zinc-700">
            <span class="text-zinc-500 text-sm"><?= __('booking.detail.notes') ?></span>
            <p class="text-sm text-zinc-800 dark:text-zinc-200 mt-1"><?= nl2br(htmlspecialchars($reservation['notes'])) ?></p>
        </div>
        <?php endif; ?>
    </div>
