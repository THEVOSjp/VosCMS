...
$classLabels = ['recurring' => '정기', 'one_time' => '1회', 'free' => '무료'];
?>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="flex items-center justify-between mb-6">
        <h1 class="text-xl font-bold text-zinc-900 dark:text-white">서비스 관리</h1>
        <a href="<?= $baseUrl ?>/service/order" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-medium">+ 서비스 신청</a>
    </div>
    ...
    <div class="mb-6 p-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl">
        <div class="flex items-start gap-3">
            ...
            <div>
                <p class="text-sm font-medium text-amber-800 dark:text-amber-200">곧 만료되는 서비스가 있습니다.</p>
    ...
    <div class="text-center py-16">
        ...
        <a href="<?= $baseUrl ?>/service/order" class="px-6 py-2.5 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm font-medium">서비스 신청하기</a>
    </div>
    <?php else: ?>
    <div class="space-y-3">
        <?php foreach ($orderGroups as $orderNum => $subs):
            ...
            $gst = $statusLabels[$groupStatus] ?? ['알 수 없음', 'bg-gray-100 text-gray-500'];
            ...
            <div class="px-4 py-4 flex items-center justify-between gap-4">
                <div class="flex-1 min-w-0">
                    <div class="flex items-center gap-2 mb-1.5">
                        ...
                        <span class="text-[10px] px-1.5 py-0.5 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400 rounded-full font-medium">만기 임박</span>
                        <?php endif; ?>
                    </div>
                    ...
                    <div class="flex items-center gap-3 mt-2 text-xs text-zinc-400">
                        ...
                        <span class="font-medium <?= $groupTotal > 0 ? 'text-zinc-600 dark:text-zinc-300' : 'text-green-600 dark:text-green-400' ?>"><?= $groupTotal > 0 ? $fmtPrice($groupTotal, $groupCurrency) : '무료' ?></span>
                        <span class="text-zinc-300 dark:text-zinc-600"><?= count($subs) ?>개 서비스</span>
                    </div>
                </div>
