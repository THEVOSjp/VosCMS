...
$pageTitle = __('admin.members.settings.tabs.general') . ' - ' . ($config['app_name'] ?? 'RezlyX') . ' Admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'update_general') {
        ...
        try {
            // 회원정보 동기화 로직
            // TODO: 실제 동기화 로직 구현
            $message = __('admin.members.settings.general.sync_complete');
            $messageType = 'success';
        } catch (Exception $e) {
            $message = __('admin.members.settings.general.sync_error') . ': ' . $e->getMessage();
            $messageType = 'error';
        }
    }
}
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-6 transition-colors mb-6">
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="py-4 border-b dark:border-zinc-700">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('admin.members.settings.general.registration_mode') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('admin.members.settings.general.registration_mode_desc') ?></p>
            <div class="flex flex-wrap gap-4 mb-3">
                ...
                <label class="flex items-center cursor-pointer">
                    <input type="radio" name="member_registration_mode" value="url_key"
                           ...
                           class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500"
                           onchange="toggleUrlKeyField()">
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('admin.members.settings.general.registration_url_key') ?></span>
                </label>
            </div>
            <!-- URL 키 입력 필드 -->
            <div id="urlKeyField" class="<?php echo $currentMode === 'url_key' ? '' : 'hidden'; ?> mt-3 p-4 bg-zinc-50 dark:bg-zinc-900 rounded-lg">
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2"><?= __('admin.members.settings.general.url_key_label') ?></label>
                <input type="text" name="member_registration_url_key"
                       ...
                       placeholder="<?= __('admin.members.settings.general.url_key_placeholder') ?>">
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-2"><?= __('admin.members.settings.general.url_key_hint') ?></p>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between py-4 border-b dark:border-zinc-700">
            <div>
                <h3 class="text-sm font-medium text-zinc-900 dark:text-white"><?= __('admin.members.settings.general.email_verification') ?></h3>
                <p class="text-sm text-zinc-500 dark:text-zinc-400"><?= __('admin.members.settings.general.email_verification_desc') ?></p>
            </div>
        ...
        <!-- 인증 메일 유효기간 -->
        <div class="py-4 border-b dark:border-zinc-700">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('admin.members.settings.general.email_validity') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('admin.members.settings.general.email_validity_desc') ?></p>
            <div class="flex items-center space-x-2">
                ...
                <span class="text-sm text-zinc-700 dark:text-zinc-300"><?= __('admin.members.settings.general.days') ?></span>
            </div>
        </div>
        ...
        <div class="flex items-center justify-between py-4 border-b dark:border-zinc-700">
            <div>
                <h3 class="text-sm font-medium text-zinc-900 dark:text-white"><?= __('admin.members.settings.general.show_profile_photo') ?></h3>
                <p class="text-sm text-zinc-500 dark:text-zinc-400"><?= __('admin.members.settings.general.show_profile_photo_desc') ?></p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
        ...
        <div class="flex items-center justify-between py-4 border-b dark:border-zinc-700">
            <div>
                <h3 class="text-sm font-medium text-zinc-900 dark:text-white"><?= __('admin.members.settings.general.logout_on_password_change') ?></h3>
                <p class="text-sm text-zinc-500 dark:text-zinc-400"><?= __('admin.members.settings.general.logout_on_password_change_desc') ?></p>
            </div>
            <label class="relative inline-flex items-center cursor-pointer">
        ...
        <!-- ID/PW 찾기 방법 -->
        <div class="py-4">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('admin.members.settings.general.password_recovery_method') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('admin.members.settings.general.password_recovery_method_desc') ?></p>
            <div class="flex flex-wrap gap-4">
                ...
                <label class="flex items-center cursor-pointer">
                    <input type="radio" name="member_password_recovery_method" value="link"
                           ...
                           class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500">
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('admin.members.settings.general.recovery_link') ?></span>
                ...
                <label class="flex items-center cursor-pointer">
                    <input type="radio" name="member_password_recovery_method" value="random"
                           ...
                           class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500">
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('admin.members.settings.general.recovery_random') ?></span>
                </label>
            </div>
        </div>
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-6 transition-colors">
    ...
    <form method="POST" onsubmit="return confirmSync()">
        ...
        <div class="flex items-center justify-between">
            <p class="text-sm text-zinc-600 dark:text-zinc-400"><?= __('admin.members.settings.general.sync_description') ?></p>
            <button type="submit" class="px-4 py-2 bg-amber-600 text-white font-medium rounded-lg hover:bg-amber-700 transition flex items-center">
                ...
                <?= __('admin.members.settings.general.sync_button') ?>
            </button>
        </div>
        <p class="mt-3 text-xs text-amber-600 dark:text-amber-400">
            ...
            <?= __('admin.members.settings.general.sync_warning') ?>
        </p>
    </form>
</div>
...
<script>
    ...
    function confirmSync() {
        return confirm('<?= __('admin.members.settings.general.sync_confirm') ?>');
    }
</script>
