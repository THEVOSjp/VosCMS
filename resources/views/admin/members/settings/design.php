...
$pageTitle = __('members.settings.tabs.design') . ' - ' . ($config['app_name'] ?? 'RezlyX') . ' Admin';
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-6 transition-colors mb-6">
    ...
    $headerTitle = __('members.settings.design.title');
    $headerDescription = __('members.settings.design.description');
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="py-4 border-b dark:border-zinc-700">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('members.settings.design.skin') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-4"><?= __('members.settings.design.skin_desc') ?></p>
            ...
            <div class="flex items-center justify-end mb-4">
                <div class="relative" id="addSkinContainer">
                    <button type="button" id="addSkinBtn"
                        ...
                        <?= __('members.settings.design.add_skin') ?? '신규 스킨 추가' ?>
                    ...
                    <div id="addSkinDropdown" class="hidden absolute right-0 mt-2 w-56 bg-white dark:bg-zinc-800 rounded-lg shadow-lg border border-zinc-200 dark:border-zinc-700 py-1 z-50">
                        <a href="#" onclick="showDirectRegisterModal(); return false;"
                           class="flex items-center px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <div>
                                <div class="font-medium"><?= __('members.settings.design.direct_register') ?? '직접 등록' ?></div>
                                <div class="text-xs text-zinc-500 dark:text-zinc-400"><?= __('members.settings.design.direct_register_desc') ?? '스킨 파일을 직접 업로드합니다' ?></div>
                            </div>
                        </a>
                        <a href="#" onclick="goToMarketplace(); return false;"
                           class="flex items-center px-4 py-3 text-sm text-zinc-700 dark:text-zinc-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors">
                            ...
                            <div>
                                <div class="font-medium"><?= __('members.settings.design.marketplace') ?? '마켓플레이스로부터 구입' ?></div>
                                <div class="text-xs text-zinc-500 dark:text-zinc-400"><?= __('members.settings.design.marketplace_desc') ?? '다양한 스킨을 찾아보세요' ?></div>
                            </div>
                        </a>
                    </div>
                </div>
                    ...
                    <div class="border-2 rounded-xl overflow-hidden transition-all
                        ...
                        <div class="p-3 bg-white dark:bg-zinc-800">
                            ...
                            <div class="flex items-center gap-1 mt-2">
                                <span class="text-xs text-zinc-400 dark:text-zinc-500"><?= __('members.settings.design.colorset') ?>:</span>
        ...
        <!-- 로그인 페이지 배경 -->
        <div class="py-4 border-b dark:border-zinc-700">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('members.settings.design.login_background') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('members.settings.design.login_background_desc') ?></p>
            <select name="member_login_background" class="w-full md:w-1/3 px-3 py-2 border border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <option value="none" <?php echo $currentBg === 'none' ? 'selected' : ''; ?>><?= __('members.settings.design.bg_none') ?></option>
                <option value="gradient" <?php echo $currentBg === 'gradient' ? 'selected' : ''; ?>><?= __('members.settings.design.bg_gradient') ?></option>
                <option value="image" <?php echo $currentBg === 'image' ? 'selected' : ''; ?>><?= __('members.settings.design.bg_image') ?></option>
                <option value="pattern" <?php echo $currentBg === 'pattern' ? 'selected' : ''; ?>><?= __('members.settings.design.bg_pattern') ?></option>
            </select>
        </div>

        <!-- 회원가입 페이지 레이아웃 -->
        <div class="py-4 border-b dark:border-zinc-700">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('members.settings.design.register_layout') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('members.settings.design.register_layout_desc') ?></p>
            <div class="flex flex-wrap gap-4">
                ...
                <label class="flex items-center cursor-pointer">
                    ...
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.layout_single') ?></span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="radio" name="member_register_layout" value="steps" <?php echo $currentLayout === 'steps' ? 'checked' : ''; ?>
                           class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500">
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.layout_steps') ?></span>
                </label>
                <label class="flex items-center cursor-pointer">
                    <input type="radio" name="member_register_layout" value="split" <?php echo $currentLayout === 'split' ? 'checked' : ''; ?>
                           class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500">
                    <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.layout_split') ?></span>
                </label>
            </div>
        </div>
        ...
        <div class="py-4 border-b dark:border-zinc-700">
            <div class="flex items-center justify-between mb-3">
                <div>
                    <h3 class="text-sm font-medium text-zinc-900 dark:text-white"><?= __('members.settings.design.social_login') ?></h3>
                    <p class="text-sm text-zinc-500 dark:text-zinc-400"><?= __('members.settings.design.social_login_desc') ?></p>
                </div>
            ...
            <div id="socialLoginOptions" class="ml-4 space-y-3 <?= !$socialLoginEnabled ? 'hidden' : '' ?>">
                ...
                <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-700/50 rounded-lg">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.social_google') ?></span>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                ...
                <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-700/50 rounded-lg">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.social_line') ?></span>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
                ...
                <div class="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-700/50 rounded-lg">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('members.settings.design.social_kakao') ?></span>
                    </div>
                    <label class="relative inline-flex items-center cursor-pointer">
        ...
        <!-- 프로필 페이지 스타일 -->
        <div class="py-4">
            <label class="block text-sm font-medium text-zinc-900 dark:text-white mb-1"><?= __('members.settings.design.profile_style') ?></label>
            <p class="text-sm text-zinc-500 dark:text-zinc-400 mb-3"><?= __('members.settings.design.profile_style_desc') ?></p>
            <select name="member_profile_style" class="w-full md:w-1/3 px-3 py-2 border border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <option value="card" <?php echo $currentProfile === 'card' ? 'selected' : ''; ?>><?= __('members.settings.design.profile_card') ?></option>
                <option value="sidebar" <?php echo $currentProfile === 'sidebar' ? 'selected' : ''; ?>><?= __('members.settings.design.profile_sidebar') ?></option>
                <option value="tabs" <?php echo $currentProfile === 'tabs' ? 'selected' : ''; ?>><?= __('members.settings.design.profile_tabs') ?></option>
            </select>
        </div>
