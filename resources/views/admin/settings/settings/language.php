...
$pageTitle = __('settings.language.page_title') . ' - ' . ($config['app_name'] ?? 'RezlyX') . ' Admin';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add_language') {
        ...
        if (empty($langCode) || empty($langNative)) {
            $message = __('settings.language.messages.error_empty');
            $messageType = 'error';
        } elseif (!preg_match('/^[a-z]{2}(_[A-Z]{2})?$/', $langCode)) {
            $message = __('settings.language.messages.error_invalid_code');
            $messageType = 'error';
        } elseif (isset($allLanguages[$langCode])) {
            $message = __('settings.language.messages.error_exists');
            $messageType = 'error';
        } else {
            // 커스텀 언어 추가
            ...
            try {
                ...
                $message = str_replace('{name}', $langNative, __('settings.language.messages.added'));
                $messageType = 'success';
            } catch (PDOException $e) {
                $message = __('settings.language.messages.error_add') . ': ' . $e->getMessage();
                $messageType = 'error';
            }
        }
    }
    ...
    if ($action === 'delete_language') {
        ...
        if (isset($customLanguages[$langCode])) {
            ...
            try {
                ...
                $message = str_replace('{name}', $deletedLang, __('settings.language.messages.deleted'));
                $messageType = 'success';
            } catch (PDOException $e) {
                $message = __('settings.language.messages.error_delete') . ': ' . $e->getMessage();
                $messageType = 'error';
            }
        }
    }
...
<div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-6 mb-6 transition-colors">
    ...
    $headerTitle = __('settings.language.title');
    $headerDescription = __('settings.language.description');
    ...
    <form method="POST" class="space-y-6">
        ...
        <div class="space-y-3">
            <div>
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2"><?= __('settings.language.auto_detect.label') ?></label>
                <div class="flex items-center gap-6">
                    <label class="flex items-center cursor-pointer">
                        ...
                        <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('settings.language.auto_detect.yes') ?></span>
                    </label>
                    <label class="flex items-center cursor-pointer">
                        <input type="radio" name="auto_detect" value="0" <?= !$autoDetect ? 'checked' : '' ?>
                               class="w-4 h-4 text-blue-600 border-zinc-300 focus:ring-blue-500">
                        <span class="ml-2 text-sm text-zinc-700 dark:text-zinc-300"><?= __('settings.language.auto_detect.no') ?></span>
                    </label>
                </div>
                <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-2"><?= __('settings.language.auto_detect.hint') ?></p>
            </div>
        </div>
        ...
        <div class="border-t dark:border-zinc-700 pt-6">
            <div class="flex items-center justify-between mb-3">
                <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300"><?= __('settings.language.supported.label') ?></label>
                <button type="button" onclick="openAddLanguageModal()"
                    ...
                    <?= __('settings.language.supported.add_button') ?>
                </button>
            </div>
            <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-4"><?= __('settings.language.supported.hint') ?></p>
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3" id="languageGrid">
                ...
                <label class="group relative flex items-center p-3 bg-zinc-50 dark:bg-zinc-900 rounded-lg cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-800 transition">
                    ...
                    <button type="button" onclick="confirmDeleteLanguage('<?= htmlspecialchars($code) ?>', '<?= htmlspecialchars($lang['native']) ?>')"
                            ...
                            title="<?= __('settings.language.delete_modal.delete') ?>"
        ...
        <!-- 기본 언어 선택 -->
        <div class="border-t dark:border-zinc-700 pt-6">
            <label for="default_language" class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2"><?= __('settings.language.default.label') ?></label>
            <p class="text-xs text-zinc-500 dark:text-zinc-400 mb-3"><?= __('settings.language.default.hint') ?></p>
        ...
        <div class="border-t dark:border-zinc-700 pt-6">
            <div class="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                <div class="flex items-start">
                    ...
                    <div>
                        <p class="text-sm font-medium text-blue-800 dark:text-blue-300"><?= __('settings.language.content_info.title') ?></p>
                        <p class="text-xs text-blue-700 dark:text-blue-400 mt-1">
                            <?= __('settings.language.content_info.description') ?>
                        </p>
                    </div>
                </div>
            </div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="relative w-full max-w-md bg-white dark:bg-zinc-800 rounded-xl shadow-xl" onclick="event.stopPropagation()">
            <div class="flex items-center justify-between p-4 border-b dark:border-zinc-700">
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-white"><?= __('settings.language.add_modal.title') ?></h3>
            ...
            <form method="POST" class="p-4 space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1"><?= __('settings.language.add_modal.code_label') ?> <span class="text-red-500"><?= __('settings.language.add_modal.required') ?></span></label>
                    <input type="text" name="lang_code" id="langCode" required
                           placeholder="<?= __('settings.language.add_modal.code_placeholder') ?>"
                    ...
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1"><?= __('settings.language.add_modal.code_hint') ?></p>
                </div>

                <div>
                    <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-1"><?= __('settings.language.add_modal.name_label') ?> <span class="text-red-500"><?= __('settings.language.add_modal.required') ?></span></label>
                    <input type="text" name="lang_native" id="langNative" required
                           placeholder="<?= __('settings.language.add_modal.name_placeholder') ?>"
                           class="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 dark:bg-zinc-700 dark:text-white rounded-lg focus:ring-2 focus:ring-blue-500">
                    <p class="text-xs text-zinc-500 dark:text-zinc-400 mt-1"><?= __('settings.language.add_modal.name_hint') ?></p>
                </div>
    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
        <div class="relative w-full max-w-sm bg-white dark:bg-zinc-800 rounded-xl shadow-xl" onclick="event.stopPropagation()">
            <div class="p-6 text-center">
                ...
                <h3 class="text-lg font-semibold text-zinc-900 dark:text-white mb-2"><?= __('settings.language.delete_modal.title') ?></h3>
                <p class="text-sm text-zinc-600 dark:text-zinc-400 mb-6">
                    '<span id="deleteLanguageName"></span>' <?= __('settings.language.delete_modal.confirm') ?><br>
                    <span class="text-red-500"><?= __('settings.language.delete_modal.warning') ?></span>
                </p>
