...
$pageTitle = __('admin.staff.schedule.title') . ' - ' . ($config['app_name'] ?? 'RezlyX') . ' Admin';
...
try {
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_weekly') {
            ...
            echo json_encode(['success' => true, 'message' => __('admin.staff.schedule.saved')]);
            exit;
        }
        ...
        if ($action === 'save_override') {
            ...
            echo json_encode(['success' => true, 'message' => __('admin.staff.schedule.override_saved')]);
            exit;
        }
        ...
        if ($action === 'delete_override') {
            ...
            if ($id) {
                ...
                echo json_encode(['success' => true, 'message' => __('admin.staff.schedule.override_deleted')]);
            } else {
...
$dayNames = [
    __('admin.staff.schedule.day_sun'),
    __('admin.staff.schedule.day_mon'),
    __('admin.staff.schedule.day_tue'),
    __('admin.staff.schedule.day_wed'),
    __('admin.staff.schedule.day_thu'),
    __('admin.staff.schedule.day_fri'),
    __('admin.staff.schedule.day_sat'),
];
?>
<!DOCTYPE html>
        ...
        <main class="flex-1 ml-64">
            ...
            <div class="p-8">
                ...
                <div class="flex items-center justify-between mb-6">
                    <div>
                        <h1 class="text-2xl font-bold text-zinc-900 dark:text-white"><?= __('admin.staff.schedule.title') ?></h1>
                        <p class="text-sm text-zinc-500 dark:text-zinc-400 mt-1"><?= __('admin.staff.schedule.description') ?></p>
                    </div>
                </div>

                <!-- 스태프 선택 -->
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-6 mb-6">
                    <label class="block text-sm font-medium text-zinc-700 dark:text-zinc-300 mb-2"><?= __('admin.staff.schedule.select_staff') ?></label>
                    <select id="staffSelect" class="w-full max-w-sm px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-sm focus:ring-2 focus:ring-blue-500">
                        <option value="">-- <?= __('admin.staff.schedule.select_staff') ?> --</option>
                ...
                <div id="scheduleArea" class="hidden space-y-6">
                    ...
                    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700">
                            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white"><?= __('admin.staff.schedule.weekly_title') ?></h2>
                        </div>
                            ...
                            <table class="w-full text-sm">
                                <thead>
                                    <tr class="bg-zinc-50 dark:bg-zinc-700/50 text-zinc-600 dark:text-zinc-300">
                                        ...
                                        <th class="px-4 py-3 text-center font-medium w-20"><?= __('admin.staff.schedule.working') ?></th>
                                        <th class="px-4 py-3 text-center font-medium"><?= __('admin.staff.schedule.start_time') ?></th>
                                        <th class="px-4 py-3 text-center font-medium"><?= __('admin.staff.schedule.end_time') ?></th>
                                        <th class="px-4 py-3 text-center font-medium"><?= __('admin.staff.schedule.break_start') ?></th>
                                        <th class="px-4 py-3 text-center font-medium"><?= __('admin.staff.schedule.break_end') ?></th>
                                    </tr>
                                </thead>
                                <tbody id="weeklyBody">
                                    <?php for ($d = 0; $d < 7; $d++): ?>
                        ...
                        <div class="px-6 py-4 bg-zinc-50 dark:bg-zinc-700/30 border-t border-zinc-200 dark:border-zinc-700 flex justify-end">
                            <button type="button" id="btnSaveWeekly" class="px-5 py-2 text-sm font-medium text-white bg-blue-600 hover:bg-blue-700 rounded-lg transition">
                                <?= __('admin.staff.schedule.save_weekly') ?>
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm overflow-hidden">
                        <div class="px-6 py-4 border-b border-zinc-200 dark:border-zinc-700 flex items-center justify-between">
                            <h2 class="text-lg font-semibold text-zinc-900 dark:text-white"><?= __('admin.staff.schedule.override_title') ?></h2>
                            <button type="button" id="btnAddOverride" class="px-3 py-1.5 text-xs font-medium text-blue-600 border border-blue-300 hover:bg-blue-50 dark:border-blue-700 dark:hover:bg-blue-900/20 rounded-lg transition flex items-center gap-1">
                                ...
                                <?= __('admin.staff.schedule.add_override') ?>
                            </button>
                        </div>
                        ...
                        <div id="overrideForm" class="hidden px-6 py-4 bg-blue-50 dark:bg-blue-900/10 border-b border-blue-200 dark:border-blue-800">
                            <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-7 gap-3 items-end">
                                <div>
                                    <label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.override_date') ?></label>
                                ...
                                <div>
                                    <label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.override_status') ?></label>
                                    <select id="ovWorking" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs">
                                        <option value="0"><?= __('admin.staff.schedule.day_off') ?></option>
                                        <option value="1"><?= __('admin.staff.schedule.working') ?></option>
                                    </select>
                                </div>
                                <div><label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.start_time') ?></label><input type="time" id="ovStart" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs"></div>
                                <div><label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.end_time') ?></label><input type="time" id="ovEnd" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs"></div>
                                <div><label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.break_start') ?></label><input type="time" id="ovBStart" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs"></div>
                                <div><label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.break_end') ?></label><input type="time" id="ovBEnd" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs"></div>
                                <div class="col-span-2 md:col-span-1">
                                    <label class="block text-xs font-medium text-zinc-600 dark:text-zinc-400 mb-1"><?= __('admin.staff.schedule.override_memo') ?></label>
                                    <input type="text" id="ovMemo" class="w-full px-2 py-1.5 border border-zinc-300 dark:border-zinc-600 rounded bg-white dark:bg-zinc-700 text-zinc-900 dark:text-white text-xs" placeholder="<?= __('admin.staff.schedule.override_memo') ?>">
                                </div>
                            </div>
                        ...
                        <div id="overrideList" class="divide-y divide-zinc-100 dark:divide-zinc-700">
                            <div class="px-6 py-8 text-center text-sm text-zinc-400"><?= __('admin.staff.schedule.no_overrides') ?></div>
                        </div>
                    </div>
                </div>
            </div>
