 ...
 */
$pageTitle = __('admin.site.widgets.mp.title') . ' - ' . ($config['app_name'] ?? 'RezlyX') . ' Admin';
...
<body class="bg-zinc-100 dark:bg-zinc-900 min-h-screen transition-colors">
    <div class="flex">
        ...
        <main class="flex-1 ml-64">
            <?php
            $pageHeaderTitle = __('admin.site.widgets.mp.title');
            ...
            <div class="p-6">
                ...
                $headerTitle = __('admin.site.widgets.mp.title');
                $headerDescription = __('admin.site.widgets.mp.description');
                $headerIconColor = '';
                $headerActions = '';
                ...
                <div class="mb-6">
                    <div class="relative max-w-md">
                        ...
                        <input type="text" placeholder="<?= __('admin.site.widgets.mp.search_placeholder') ?>"
                ...
                <div class="flex items-center gap-2 mb-6">
                    <button class="px-4 py-2 rounded-lg text-sm font-medium bg-blue-600 text-white"><?= __('admin.site.widgets.categories.all') ?></button>
                    <button class="px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition"><?= __('admin.site.widgets.mp.featured') ?></button>
                    <button class="px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition"><?= __('admin.site.widgets.mp.popular') ?></button>
                    <button class="px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition"><?= __('admin.site.widgets.mp.recent') ?></button>
                    <button class="px-4 py-2 rounded-lg text-sm font-medium bg-white dark:bg-zinc-800 text-zinc-600 dark:text-zinc-300 hover:bg-zinc-50 dark:hover:bg-zinc-700 transition"><?= __('admin.site.widgets.mp.free') ?></button>
                </div>
                ...
                <div class="bg-white dark:bg-zinc-800 rounded-xl shadow-sm p-16 text-center">
                    ...
                    <h3 class="text-xl font-bold text-zinc-900 dark:text-white mb-2"><?= __('admin.site.widgets.mp.coming_soon') ?></h3>
                    <p class="text-zinc-500 dark:text-zinc-400 mb-6"><?= __('admin.site.widgets.mp.coming_soon_desc') ?></p>
                        ...
                        <div class="bg-zinc-50 dark:bg-zinc-700/50 rounded-xl p-4 text-left border border-zinc-200 dark:border-zinc-600">
                            ...
                            <div class="flex items-center justify-between mt-3">
                                <span class="text-sm font-bold <?= $sw['price'] > 0 ? 'text-blue-600 dark:text-blue-400' : 'text-green-600 dark:text-green-400' ?>">
                                    <?= $sw['price'] > 0 ? '$' . number_format($sw['price'], 2) : __('admin.site.widgets.mp.free') ?>
                                </span>
                                <span class="text-xs text-zinc-400"><?= number_format($sw['downloads']) ?> <?= __('admin.site.widgets.mp.downloads') ?></span>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
